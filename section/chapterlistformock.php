...
<div class="container-fluid actualcontent mb-5">
  ...
  <div class="row">
     <div class="col-xl-8 col-lg-8 col-md-10 mx-auto">
       ...
       <p class="colortheme subjectcardcontent" style="letter-spacing: 2px;">MOCK INSEM QUESTION PAPERS</p>
       <hr>
     </div>
   </div>
   <?php
         $i = 1;
         $insem = "SELECT * FROM `mock_master` WHERE `subject_id` = '$subject_id' AND `status` = 0";
...
<div class="row">
     <div class="col-xl-8 col-lg-8 col-md-10 mx-auto">
       <p class="colortheme subjectcardcontent" style="letter-spacing: 2px;">MOCK ENDSEM QUESTION PAPERS</p>
       <hr>
     </div>
   </div>
   <?php
         $i = 1;
         $insem = "SELECT * FROM `mock_master` WHERE `subject_id` = '$subject_id' AND `status` = 1";
         $res_insem = mysqli_query($con,$insem);
         $count = mysqli_num_rows($res_insem);
         if($count > 0){
        while ($row_insem = mysqli_fetch_array($res_insem)) {
